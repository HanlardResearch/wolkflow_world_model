...
class Reasoning_Agent(Agent):
    ...
    def _clean_final_answer(self, answer):
        ...
        answer = re.sub(r"\*Your previous reasoning was:.*$", "", answer, flags=re.DOTALL).strip()
        answer = re.sub(r"\[/?CONTEXT_FROM_SYSTEM\]", "", answer).strip()
        answer = answer.splitlines()[0].strip() if answer else ""
        return answer

    def _extract_final_answer(self, raw_response):
        match = re.search(r"FINAL ANSWER:\s*(.+)", raw_response)
        if not match:
            return ""
        return self._clean_final_answer(match.group(1))
