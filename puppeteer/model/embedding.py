...
class RewardModelTokenRepresentation():
    def __init__(self):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(MODEL_WEIGHT_PATH, torch_dtype=torch.bfloat16, device_map="cuda:0", trust_remote_code=True)
    ...
    def __call__(self, messages:List):
        with torch.no_grad():
            ...
            # Ensure there is a user message for the chat template
            ...
            # Qwen and other templates strictly require system messages to be ONLY at the beginning
            ...
            tokenized_message = self.tokenizer.apply_chat_template(messages, tokenize=True, add_generation_prompt=False, return_tensors="pt", return_dict=True, max_length=12800)
            model_log_and_print("tokenized done")
            input_ids = tokenized_message['input_ids'].to(self.model.device)
            attention_mask = tokenized_message['attention_mask'].to(self.model.device)
