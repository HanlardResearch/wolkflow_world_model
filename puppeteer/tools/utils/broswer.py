...
class SimpleTextBrowser:
    ...
    def __init__(
        ...
        tavily_api_key: Optional[Union[str, None]] = None,
        ...
        if tavily_api_key is None:
            self.tavily_api_key = global_config.get("api_keys").get("tavily_api_key")
    ...
    def set_address(self, uri_or_path: str) -> None:
        ...
        elif uri_or_path.startswith("tavily:"):
            self._tavily_search(uri_or_path[len("tavily:") :].strip())
        else:
            if (
    ...
    def _split_pages(self) -> None:
        ...
        if self.address.startswith("tavily:"):
    ...
    def _tavily_search(self, query: str) -> None:
        from tavily import TavilyClient

        # Check the cache
        if self.tavily_cache is not None:
            cached = self.tavily_cache.get(query)
        ...
        if results is None:
            ...
            # Make the request via TavilyClient
            try:
                client = TavilyClient(self.tavily_api_key)
                results = client.search(query=query, search_depth="advanced")
            except Exception as e:
                raise requests.exceptions.RequestException(f"Failed to fetch Tavily search results: {e}")

            # Cache the results
            if self.tavily_cache is not None:
                self.tavily_cache.set(query, results)
        ...
        for item in results.get("results", []):
            ...
            web_snippets.append(
                f"{idx}. [{title}]({url})\n{_prev_visit(url)}{content_text}"
            )

        self.page_title = f"{query} - Search"

        content = (
            f"A Tavily search for '{query}' found {len(web_snippets)} results:\n\n## Web Results\n"
            + "\n\n".join(web_snippets)
        )
