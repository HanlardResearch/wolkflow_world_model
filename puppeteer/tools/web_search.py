...
#
#             if len(results) == 0:
#                 results = "Page not exists in arXiv, try different search tools like Tavily search."
...
class arXiv_SearchEngine(Web_Search):
    ...
    def search(self, query):
        try:
            ...
            # Perform the search without the deprecated session parameter
            search = arxiv.Search(
                query=query,
                max_results=5,
                sort_by=arxiv.SortCriterion.Relevance
...
@global_tool_registry("search_tavily")
class Tavily_SearchEngine(Web_Search):
    def __init__(self, name):
        super().__init__()
        self.name = name
    
    def search(self, query):
        self.broswer.set_address("tavily:"+query)
        if self.broswer.page_content != None and len(self.broswer.page_content) != 0:
            return True, self.broswer.page_content
        else:
            return False, "page not exists in tavily, try different search tools"
